...
func PutMetadata(name string, version int, size int64, hash string) error {
	...
	if r.StatusCode != http.StatusCreated {
		result, _ := ioutil.ReadAll(r.Body)
		return fmt.Errorf("fail to put metadata: %d %s with %s", r.StatusCode, string(result), request)
	}
	return nil
}
...
func SearchAllObjects() ([]string, error)  {
	...
	json.Unmarshal(result, &sr)
	for i := range sr.Hits.Hits {
		objs = append(objs, sr.Hits.Hits[i].Source.Name)
	}
	return objs, nil
}
